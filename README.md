...
- **Performance** — your numbers in four labelled groups: **Form & load** (the load model, multi-week trends
  ...
  Durability is AI Endurance's DFA-α1 index from **archived** sessions — AIE's 2026-08 connector change
  stopped sending per-activity durability values (spec 10), so the trend runs on history and a sport with no
  values shows an explanatory note rather than a silently missing row. In **Career & PBs**, *Best ride power* is normalised power (NP) over a whole ride ≥20 km — spike-weighted,
...
**`npm run weekly`** (or when the scheduled job runs them). See [SETUP.md → "Season arc"](./SETUP.md) and
...
| --- | --- | --- |
| `sync` / `get_state` | assemble (or read) today's AthleteState — plan, recovery, HRV/RHR, weight, thresholds, zones. `sync` also **auto-fetches recent raw `.FIT` streams** (parity with the dashboard Sync) and both surface a **granular-data completeness** readout — which recent sessions are missing their raw `.FIT` (so their per-interval splits / biomechanics are unreachable) and *why* (Garmin off / not reachable / download capability missing / a download that failed), never a silent zero. `get_state` reads the last snapshot and **flags it with a ⚠ STALE line if it's from a previous day** (run `sync` to refresh); for an interpreted "how am I today" verdict use `readiness`, not the raw dump | none |
