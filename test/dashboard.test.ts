...
test("every inline <script> in the dashboard is syntactically valid JS", () => {
  const html = render();
  // Case/attribute-tolerant <script> match (gi + \b[^>]*) so CodeQL's js/bad-tag-filter is happy.
  // The dashboard only emits lowercase, attribute-less <script>, so the matched blocks are unchanged.
