...
|---|---|---|
| AI Endurance auth | `token present (last saved Nd ago; refreshed on demand)` | `✗ no cached token` → `npm run auth:aie` (host-only, opens a browser). File presence only — read the next line too. |
| AI Endurance live read | `getUser answered — the cached token works` | `✗ re-auth needed (token missing or rejected)` → `npm run auth:aie`. This is the honest check: AIE answers `initialize`/`tools/list` WITHOUT a token, so "connected" and a present token file prove nothing. |
| AI Endurance retired tokens | (absent) | `· N rejected-token copies kept (latest aie-tokens.revoked-<time>.json)` → a refresh got `invalid_grant` at that time (see the token-loss row below); delete the copies once understood. |
...
| `AI Endurance re-auth needed` / 401 / OAuth error | Cached AIE OAuth token missing or unrefreshable | `npm run doctor` (the "AI Endurance live read" line) or `npm run health-remote` | `cd /Users/maxeskell/dev/personal-training-app && npm run auth:aie` — **host-only** (waits on `http://localhost:8765/callback`; cannot be done headless/cloud). It prints "✓ authorized" only after an authenticated read; a bare "connected" is not proof. Auth failures are surfaced as-is, never retried. |
| The token file vanished / an `aie-tokens.revoked-<time>.json` appeared / the dashboard shows a days-old "Last session" under a fresh "Data last updated" | A refresh was answered `invalid_grant` (dead refresh token — typically a 06:00 `ping` suspended by sleep mid-refresh: the reply was lost, the on-disk token rotated away) and the SDK retired the file; every AIE read since fails but the assemble still saves an all-null state with a fresh `assembledAt` | `ls -la ~/.endurance-coach/` (dir mtime = the moment) · `grep -h "aie-oauth" reports/*.log \| tail` (the audit line says which pid/when) · `pmset -g log \| grep -i "DarkWake\|Sleep" \| grep "<that day>"` (was the Mac dozing?) · `grep -n "connect timed out" reports/ping.log \| tail -3` | Re-auth (row above), then run `npm run catch-up` to backfill the gap; delete or regenerate anything written blind meanwhile (states with all-null AIE slots, an empty weekly brief). Story + open plan: `docs/specs/improvements/11-aie-token-loss-and-blind-reauth.md`. |
