...
| `npm run auth:aie` | AI Endurance OAuth — opens a browser when an authenticated read (`getUser`) is refused, and reports success only once that read works; re-run it whenever `doctor` says re-auth needed. Tokens cached in `~/.endurance-coach`; a token the server rejects is retired to `aie-tokens.revoked-<time>.json`, never deleted |
...
| `npm run archive:heal:install` (`-- CHUNK INTERVAL`) | schedule the auto-heal (every 6h by default); `archive:heal:uninstall` to stop |
...
| `npm run career:build -- --tp <csv> --races <json> --fit-dir <dir>` | build the gitignored `data/career-history.json` behind Career & PBs from your TrainingPeaks export + activity files (see SETUP.md → "Career history"); re-running keeps the races already in the file |
| `npm run race:result -- --date YYYY-MM-DD --type "Olympic triathlon" [--event …] [--location …] [--file results.txt] [--dry-run]` | paste an **official** result (your row + Swim/T1/Bike/T2/Run splits off the results page — the table header + row, or the vertical `Key: value` block — via `--file` or stdin) into the career history, keyed by date: official finish, placing and leg times land, FIT-derived HR/power on matching legs are kept, and the next dashboard render scores the frozen pre-race prediction against it. Deterministic, no network, nothing scraped |
