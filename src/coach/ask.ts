...
export function buildAskContext(state: AthleteState, insights: InsightReport): string {
  ...
  return [
    ...
    `- Monitoring rule (backtested): ${ins.monitoring.best ? `${ins.monitoring.best.name}, lead ${ins.monitoring.best.lead}d, hit ${Math.round(ins.monitoring.best.hitRate * 100)}%/false-alarm ${Math.round(ins.monitoring.best.falseAlarmRate * 100)}%` : "none with skill yet"}`,
