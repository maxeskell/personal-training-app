...
export async function answerQuestion(llm: CoachLLM, question: string, state: AthleteState, archive?: ArchiveInput): Promise<AskResult> {
  ...
  const prompt = [
    ...
    "`recommendations` with the fewest genuinely distinct, family-tagged, single-line imperatives the athlete can",
    "act on — merge anything that is the same underlying action, and prefer one strong line over restating it; for",
    "a purely informational question leave it empty.",
    "",
    `QUESTION: ${question}`,
    "",
    "=== DATA ===",
