...
export function insightMetricsSummary(ins: InsightReport): string {
  ...
  return [
    ...
    `- Same-day run/ride decoupling (Q4 — proxy, not a true off-bike transition): ${ins.brick.decouplingPct != null ? `run EF ${ins.brick.decouplingPct}% lower on same-day run+ride days vs run-only days (${ins.brick.brickDays} same-day days)` : "insufficient power-equipped runs"}`,
