...
export async function runSeasonNarrative(
  ...
  const prompt = [
    ...
    "seasons, not weeks.",
    "",
    "Write it as a COHESIVE COACH'S READ — flowing prose a coach would actually say, not a disconnected",
    "bullet list. EVERY claim must be SUPPORTED by the athlete's own numbers below (cite them inline — CTL",
    "now/target/trend, the gap, the peak-year benchmark, the lever statuses, the flags); a point without a",
    "number behind it doesn't belong. Anything you estimate, label it. Don't invent numbers not given.",
    "",
    "LEAD with the single biggest multi-season lever for THIS athlete (not a generic list), with the number",
    "that makes it the biggest. Then give a RANKED set of structural levers (most leverage first), each tied",
    "to a figure. Be honest about trade-offs — especially: a GLP-1 helps short-term W/kg by dropping weight",
    "but risks muscle/bone if strength + fuelling slip; stale bloods mean managing that blind. Distinguish the",
    "patient, compounding levers (chronic load, swim technique, strength) from quick fixes. Close with a",
    "concrete SEASON-BY-SEASON ARC (this year → next → the year after) showing how the phases sequence to the",
    "goal.",
    "",
